...
const Points = () => {
    ...
    useEffect(() => {
      async function loadPosition() {
        ...
        if (status !== 'granted') {
          Alert.alert('Erro', 'Precisamos de sua permissão pra obter sua localização');
          return;
        }
              ...
              <View style={styles.mapContainer}>
                  { initialPosition[0] !== 0 && (
                    <MapView style={styles.map}
                      initialRegion={{ latitude: initialPosition[0], longitude: initialPosition[1], 
                                      latitudeDelta: 0.014, longitudeDelta: 0.014 }}>
                      {points.map(point => (
                        <Marker key={String(point.id)} style={styles.mapMarker} onPress={() => handleNavigateToDetail(point.id)} 
